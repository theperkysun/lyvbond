...
export default function DailyBody() {
  ...
  const mapUserToCard = (user) => {
    ...
    return {
      ...
      img: user.profileImage ? { uri: user.profileImage } : null, // Assuming card handles source object
      ...
      ...user,
      location: locationString, // Ensure string wins over object
  ...
  const fetchDailyMatches = async () => {
    try {
      ...
      console.log("Daily users fetch error", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLike = () => {
    ...
    // Animation is triggered in onSwipedRight to avoid double animation if needed, 
    // but user asked for button functionality. 
    // Usually visual feedback is immediate on button press.
    // Let's trigger it here.
    // handleHeartPress(); // Moved to onSwipedRight to sync with action or keep here? 
    // If I put it here, it shows immediately. If I put in onSwipedRight, it shows after swipe starts.
