...
import { userProfileData } from '../CommonComponents/profileUserData';

const { width, height } = Dimensions.get('window');

export default function MatchesProfileCard({
  card = {},
  onPress,
  ...
  if (typeof location === 'string') {
    ...
    const parts = [location.subcommunity, location.city, location.state, location.country].filter(Boolean);
    // Join up to 2 parts for brevity, e.g. "Subcommunity, City" or "City, Country"
    displayLocation = parts.slice(0, 2).join(', ');
  }

  let galleryImages = [];
  const mainUrl = card.profileImage || url;
  if (mainUrl) {
    galleryImages.push(typeof mainUrl === 'string' ? { uri: mainUrl } : mainUrl);
  }

  if (Array.isArray(images)) {
    images.forEach(img => {
      const imgObj = typeof img === 'string' ? { uri: img } : img;
      if (imgObj && imgObj.uri) {
        // Prevent duplicate profile image
        const isDup = galleryImages.some(existing => existing.uri === imgObj.uri);
        if (!isDup) {
          galleryImages.push(imgObj);
        }
      } else if (imgObj && typeof imgObj === 'number') {
        // Local require images (like require('./image.png'))
        const isDup = galleryImages.some(existing => existing === imgObj);
        if (!isDup) galleryImages.push(imgObj);
      }
    });
  }

  const actualPhotosCount = galleryImages.length;
  ...
  return (
    <>
      <TouchableOpacity activeOpacity={100} style={[styles.cardWrapper, containerHeight ? { height: containerHeight } : null]} onPress={onPress}>
        ...
        <View style={styles.topRight}>
          ...
          >
            ...
            <Text style={styles.photoText}>{actualPhotosCount}</Text>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  cardWrapper: {
    ...
    overflow: 'hidden',
    marginVertical: 0,
    backgroundColor: '#1a1a1a', // Fallback background color if image is missing/loading
  },
